export default class LAA {
    ...
    static probe(buffer) {
        const header = new Buffer.from(buffer.buffer.slice(0, 3)).toString();
                            ...
                            switch (ctrl) {
                                ...
                                case 0x67:
                                    if ((this.adlib_style & this.CMF_STYLE) != 0) {
                                        this.adlib_mode = vel;
                                        if (this.adlib_mode == this.ADLIB_RHYTHM) this.midi_write_adlib(0xbd, this.adlib_data[0xbd] | (1 << 5));
