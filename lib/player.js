...
class Player extends Readable {
    ...
    load(buffer, callback, postMessage) {
        console.log(postMessage)
        if (!this.options.disableWorker && process.browser && typeof window != 'undefined' && 'Worker' in window) {
            this._load_worker(buffer, callback, postMessage);
        } else {
            this._load_internal(buffer, callback, postMessage);
        }
    }

    // Runs either on main thread or inside Worker
    // Process the whole music file and generate buffer
    _load_internal(buffer, callback, postMessage) {
        try {
            ...
            this.initPostMessage(postMessage);
    ...
    _load_worker(buffer, callback, postMessage) {
        this.initPostMessage(postMessage);
