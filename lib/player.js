...
class Player extends Readable {
    ...
    constructor(format, options) {
        ...
        this.#options.bufferSize = this.#options.bufferSize || 128;
    ...
    worklet_update(outputs) {
        ...
        var seek = 0, len = this.#samplesBuffer.length;
        this.#samplesBuffer.fill(0.0);

        if (this.#chunkSize === 0) {
            this.worklet_player.update();
            this.#chunkSize = 2 * ((this.#sampleRate * this.worklet_player.refresh()) | 0);
        }

        if (this.#chunkSize < this.#options.bufferSize * 2) {
            this.worklet_player.opl.read(this.#samplesBuffer, seek, this.#chunkSize);
            seek += this.#chunkSize;
        ...
        if (this.#chunkSize > 0) {
            ...
            this.worklet_player.opl.read(this.#samplesBuffer, seek);
